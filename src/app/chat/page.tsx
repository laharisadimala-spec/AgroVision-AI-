...
import { Send, Bot, User, Mic, MicOff, Volume2, VolumeX, Loader2, Sparkles } from 'lucide-react';
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { 
        role: 'model', 
        content: translate('error_chat') || 'Sorry, I encountered an error. Please try again.' 
      }]);
    } finally {
      setIsLoading(false);
    }
      ...
      <main className="flex-1 lg:ml-[300px] flex flex-col h-screen relative overflow-hidden">
        ...
        <div className="p-4 sm:p-6 pb-2 border-b border-sage-200 bg-white/50 backdrop-blur-xl z-10 sticky top-0 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight text-earth-900">{translate('ai_assistant') || 'AI Assistant'}</h1>
              <p className="text-earth-500 font-medium text-sm">{translate('ai_desc') || 'Voice-enabled farming expert'}</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 z-10 relative">
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center max-w-2xl mx-auto">
              ...
              <h2 className="text-3xl font-extrabold text-earth-900 mb-4">{translate('how_can_i_help') || 'How can I help your farm today?'}</h2>
              <p className="text-earth-600 text-lg mb-10">{translate('ask_anything') || 'Ask about crop diseases, fertilizers, or weather impacts in your language.'}</p>
            ...
            <div className="space-y-6 max-w-4xl mx-auto pb-4">
              <AnimatePresence initial={false}>
                {messages.map((msg, idx) => (
                  ...
                  >
                    <div className={`shrink-0 p-3 rounded-2xl shadow-sm ${
                      msg.role === 'user' ? 'bg-earth-100 text-earth-700' : 'bg-leaf-100 text-leaf-600'
                    }`}>
                      {msg.role === 'user' ? <User size={24} /> : <Bot size={24} />}
                    </div>
                    <div className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                      <div className={`px-6 py-4 rounded-[24px] shadow-sm max-w-[85%] text-lg font-medium leading-relaxed ${
                        msg.role === 'user' 
          ...
          <form onSubmit={handleSubmit} className="max-w-4xl mx-auto relative flex items-center gap-3">
            ...
            <div className="relative flex-1">
              <input
                ...
                placeholder={isListening ? (translate('listening') || 'Listening...') : (translate('type_message') || 'Ask a farming question...')}
